...
int atm (int bank_out_fd, int atm_in_fd, int atm_id, Command *cmd) {
  ...
  if (atm_id==i){
    if (c==CONNECT || c==EXIT ||c==TRANSFER || c==DEPOSIT ||c==WITHDRAW || c==BALANCE){
...
/*if (c == 1) {
  ...
  if ((status=checked_write(status)) != SUCCESS){
    return status;
  }
  else {
    status=read(atm_in_fd,atmcmd,MESSAGE_SIZE);
    if ((status=checked_read(status))!=SUCCESS){
      return status;
    }
...
else if (atmcmd.cmd[0] ==7){
  status= ERR_NOFUNDS;

}
else if (atmcmd.cmd[0] ==9){
  status=ERR_UNKNOWN_ACCOUNT;
}
else {
  status = ERR_UNKNOWN_CMD;
  return error_msg(ERR_UNKNOWN_CMD, "unknown command");
  }
