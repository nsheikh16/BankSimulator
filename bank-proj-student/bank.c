...
if (c==TRANSFER){
  int result1= check_valid_account(f);
  int result2= check_valid_account(t);
  if (result1==SUCCESS ){
    if (accounts[f]<a){
...
if (c==BALANCE){
  ...
  error_msg(ERR_UNKNOWN_CMD, "message recieved from unknown ATM");
  result= ERR_UNKNOWN_CMD;
}

if (c!= CONNECT && c!=WITHDRAW && c!= EXIT && c!=DEPOSIT && c!= TRANSFER && c!= BALANCE){
